...
m.bass_effect = Math.max((Math.max(m.bass, m.bass_att)-1.18), 0);
...
m.zoom = ((m.zoom-(((bnot(m.grid)*Math.atan2(((m.x*m.q3)*3.14), ((m.y*m.q5)*3.14)))*0.14)*((m.x*2)-1)))+(0.12*m.bass_effect));
...
m.sy = (m.sy+(((0.2*m.q2)*Math.sin((m.y*1.6)))*(1-m.grid)));
	...
	'pixel_eqs_str' : ('bass_effect = max(max(bass,bass_att)-1.18,0);\n' + 'grid=rad*10%(above(q1,q4)+above(q2,q5)+above(q3,q4)) + (sqrt(2)-rad)*10%(above(q1,q5)+above(q2,q4)+above(q3,q5))*q1;\n' + 'rot=if(grid,.12*cos(rad*3.14+x*q1*3.14+y*q2*3.14)*(q5+q4),0);\n' + 'zoom=zoom-bnot(grid)*atan2(x*q3*3.14,y*q5*3.14)*.14*(x*2-1)+0.12*bass_effect;\n' + 'sx=sx+.2*q1*sin(x*1.6)*-grid;\n' + 'sy=sy+.2*q2*sin(y*1.6)*(1-grid);'),
};
