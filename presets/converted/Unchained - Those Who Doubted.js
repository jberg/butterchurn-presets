...
m.q8 = m.entropy;
m.ob_r = (0.5+(0.5*Math.sin(((m.time*2.157)+m.q6))));
m.ob_b = (0.5+(0.5*Math.sin(((m.time*1.689)+m.q5))));
m.ob_g = (0.5+(0.5*Math.sin(((m.time*0.413)+m.q4))));
...
m.wave_r = ifcond(m.treb_changed, (0.5+(0.5*m.q3)), ifcond(m.bass_changed, (0.5+(0.5*m.q4)), (0.5+(0.5*m.q1))));
m.wave_g = (0.5+(ifcond(bnot(mod(m.q8,2)), m.q1, ifcond(bnot(mod(m.q8,3)), m.q2, m.q3))*0.5));
m.wave_b = ifcond(bnot(mod(m.q8,6)), (0.5+(0.5*m.q5)), ifcond(m.mid_changed, m.bass_changed, (0.5+(0.5*m.q1))));
m.wave_mode = (((m.q8-m.bass_changed)+m.mid_changed)+m.treb_changed);
m.wave_mystery = mod(m.frame,2);
m.wave_x = (m.wave_x+(0.02*m.q1));
m.wave_y = (m.wave_y+(0.02*m.q2));
		m.rkeys = ['sx','sy','zoom','dx','dy'];
		return m;
	},
	'pixel_eqs' : function(m) {
m.yep = ifcond(above(m.q5, 0), above(m.x, 0.5), below(m.x, 0.5));
m.nope = ifcond(above(m.q4, 0), above(m.y, 0.5), below(m.y, 0.5));
m.maybe = ifcond(above(m.q6, 0), m.yep, m.nope);
m.sometimes = ifcond(above(m.q6, 0), bnot(m.yep), bnot(m.nope));
