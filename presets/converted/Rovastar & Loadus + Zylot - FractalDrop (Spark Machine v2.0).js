...
m.zoom = 1;
m.zoom = bitand((m.zoom+0),pow(((m.rad*6)+(m.bass*Math.sin(((m.ang*4)+(m.time*10))))), (m.x*m.y)));
		return m;
	},
	'waves' : [
		{
...
m.y = (m.y+(0.1*Math.sin(((m.q1*0.5)+2))));
	...
	'pixel_eqs_str' : ('zoom = 1;\n' + 'zoom = zoom + 0&pow(rad*6+(bass)*sin(ang*4+time*10),x*y);'),
};
