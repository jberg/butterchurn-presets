...
m.zoom = (m.zoom-(0.55+(0.1*Math.sin(m.time))));
m.zoom = (m.zoom+Math.abs(Math.sin((((((m.rad*m.rad)*m.rad)*m.bass)*8)+(m.time*5)))));
m.zoom = sqrt(pow(m.zoom, (bitand(0,(m.rad*1.4))*0.3)));
		return m;
	},
	'waves' : [
		{
	...
	'pixel_eqs_str' : ('zoom = zoom - (.55+.1*sin(time));\n' + 'zoom = zoom + abs(sin(rad*rad*rad*bass*8+time*5));\n' + 'zoom = sqrt(pow(zoom,(0&rad*1.4)*.3));'),
};
