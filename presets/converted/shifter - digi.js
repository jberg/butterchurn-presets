...
m.t8 = (1+(((rand(101)*0.01)-(rand(101)*0.01))*0.3));
			m.rkeys = ['hh','sw','hr','gr','it','fr','er','dr','cr','br','ar','hang','gang','fang','eang','ita','dang','cang','bang','ha','aang','hb','hc','hd','he','lev','hf','hg'];
			return m;
		},
		'frame_eqs' : function(m) {
m.tm = (m.time*0.1);
...
m.len = m.t4;
m.mad = 0.6;
m.it = ifcond(above(m.sample, 0), (m.it+equal(m.lev, 8)), 1);
...
m.sw = (m.sw-1);
m.sw = (ifcond(equal(m.lev, 8), m.mod, m.sw)*above(m.sample, 0));
m.lev = ifcond(above(m.sample, 0), ifcond(above(m.sw, 0), (m.lev-1), (m.lev+1)), 0);
m.a = ((m.lev*0.1)*1.46);
m.a = equal(m.lev, 8);
...
m.hr = ifcond(above(m.sample, 0), m.hr, 1);
m.hr = ifcond(equal(m.lev, 7), (m.hr*-1), m.hr);
...
m.hh = ifcond(m.swi, (1-(2*rand(2))), m.hh);
...
m.hang = ifcond(above(m.sample, 0), m.hang, (m.hang+((m.q3*m.hh)*3)));
m.len = (m.len*m.mad);
...
m.an = ((m.hang*(((((((m.ar+m.br)+m.cr)+m.dr)+m.er)+m.fr)+m.gr)+m.hr))*m.hr);
m.len = (m.len*m.mad);
m.x = (m.x+(((above(m.lev, 7)*Math.sin(m.an))*m.len)*m.hr));
m.y = (m.y+((above(m.lev, 7)*Math.cos(m.an))*m.len));
m.x = (((m.x-0.5)*0.75)+0.5);
m.x = (Math.floor((m.x*200))*0.005);
m.y = (Math.floor((m.y*200))*0.005);
		return m;
	},
...
m.poly = 5;
		...
		'frame_eqs' : function(m) {
m.r = (0.75+(0.25*Math.sin((m.time*1.187))));
m.g = (0.75+(0.25*Math.sin((m.time*1.153))));
m.b = (0.75+(0.25*Math.sin((m.time*1.105))));
m.r2 = (1.5-m.r);
m.g2 = (1.5-m.g);
m.b2 = (1.5-m.b);
		return m;
		...
		'frame_eqs_str' : ('r = .75 + .25*sin(time*1.187);\n' + 'g = .75 + .25*sin(time*1.153);\n' + 'b = .75 + .25*sin(time*1.105);\n' + 'r2 = 1.5 - r;\n' + 'g2 = 1.5 - g;\n' + 'b2 = 1.5 - b;'),
