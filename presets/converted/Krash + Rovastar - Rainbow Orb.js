...
m.dx = -0.0005;
m.dy = -0.0005;
m.wave_x = (0.5+(0.3*Math.sin(((m.bass+m.treb)+m.mid))));
m.wave_r = (1+Math.sin((-m.wave_x*6.28)));
m.wave_g = Math.abs(Math.sin(((2*m.wave_x)*6.28)));
m.wave_b = Math.sin((m.wave_x*6.28));
...
m.rot = (m.rot+ifcond(equal(Math.sin(m.ang), 0), m.rot, div(Math.sin((1-m.rad)),2)));
		return m;
	},
	'warp' : (''),
	'comp' : (''),
	...
	'pixel_eqs_str' : ('rot = rot + if(equal(sin(ang), 0), rot, sin(1-rad)/2);'),
};
