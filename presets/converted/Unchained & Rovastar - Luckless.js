...
m.wave_mode = m.q7;
m.zoom = (m.zoom+(0.1*Math.sin((m.time*0.8))));
		m.rkeys = ['zoom','rot'];
		return m;
	},
	'pixel_eqs' : function(m) {
m.rot = (m.rot+ifcond(below(Math.sin((m.ang+(Math.sin((m.rad+(0.5*m.q4)))*5))), m.q6), -m.rot, (0.5*Math.sin((((m.rad+(0.5*m.q1))*4)*m.q5)))));
m.zoom = (m.zoom+(m.rot*m.q3));
		return m;
	},
	'warp' : (''),
	'comp' : (''),
	...
	'pixel_eqs_str' : ('rot = rot + if(below(sin(ang+sin(rad+.5*q4)*5), q6), -rot, 0.5*sin((rad+.5*q1)*4*q5));\n' + 'zoom=zoom+rot*q3;'),
};
