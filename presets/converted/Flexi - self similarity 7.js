...
m.t8 = -m.t8;
m.y = (m.sample*0.21);
m.x = (((0.5+(m.t8*0.01))-((m.t8*m.sample)*0.0025))+(((sqr(((m.sample*2)-1))-1)*m.q1)*0.5));
m.pi3 = ((3.1415*2)*0.3333);
...
m.b = ifcond(below(m.b, 0), 0, m.b);
		...
		'init_eqs_str' : ('t2 = 0;\n' + 't3 = 0;\n' + 't4 = 0;\n' + 'cl = 0;'),
		'frame_eqs_str' : ('t8 = 1;'),
		'point_eqs_str' : ('t8 = -t8;\n' + 'y = sample*0.21;\n' + 'x = 0.5 + t8*0.01 - t8*sample*0.0025 + (sqr(sample*2-1)-1)*q1*0.5;\n' + 'pi3 = 3.1415*2*0.3333;\n' + 't = -q3*300+sample*asin(1)*4.01;\n' + 'c=1;\n' + 'r = if(above(r,1),1,r);\n' + 'r = if(below(r,0),0,r);\n' + 'g = if(above(g,1),1,g);\n' + 'g = if(below(g,0),0,g);\n' + 'b = if(above(b,1),1,b);\n' + 'b = if(below(b,0),0,b);'),
...
m.w = 0;
		...
		'frame_eqs' : function(m) {
m.w = ((-Math.atan2(0.5, m.q1)*8)+(Math.asin(1)*4));
m.ang = m.w;
m.x = (0.5+((Math.sin(m.w)*0.19)*1.5));
m.y = (0.2+((Math.cos(m.w)*0.26)*1.5));
		return m;
	},
		'init_eqs_str' : (''),
		'frame_eqs_str' : ('w = -atan2(0.5,q1)*8+asin(1)*4 ;\n' + 'ang = w;\n' + 'x = 0.5 +sin(w)*0.19*1.5;\n' + 'y = 0.2 +cos(w)*0.26*1.5;'),
...
m.w = 0;
		...
		'frame_eqs' : function(m) {
m.w = (((Math.atan2(0.5, m.q1)*4)+(Math.asin(1)*4))+div(Math.asin(1),3));
m.ang = m.w;
m.x = (0.5+(Math.sin(m.w)*0.19));
m.y = (0.02+(Math.cos(m.w)*0.26));
m.rad = 0.5;
m.zoom = 0.5;
		return m;
	},
		'init_eqs_str' : (''),
		'frame_eqs_str' : ('w = atan2(0.5,q1)*4+asin(1)*4 + asin(1)/3;\n' + 'ang = w;\n' + 'x = 0.5 +sin(w)*0.19;\n' + 'y = 0.02 +cos(w)*0.26;\n' + 'rad = 0.5;\n' + 'zoom = 0.5;'),
