...
m.q8 = m.entropy;
m.ob_r = (0.4+(0.4*Math.sin(((m.time*2.157)+m.q6))));
m.ob_b = (0.8+(0.2*Math.sin(((m.time*1.689)+m.q5))));
m.ob_g = (0.6+(0.4*Math.sin(((m.time*0.413)+m.q4))));
m.ib_r = (0.5+(0.5*Math.cos(((m.time*1.2)+(m.q1*0.1)))));
m.ib_b = (0.4+(0.4*Math.cos(((m.time*2.811)+(m.q2*0.1)))));
m.ib_g = (0.4+(0.4*Math.cos(((m.time*1.666)+(m.q3*0.1)))));
m.ib_size = (0.05+(0.03*m.q2));
...
m.wave_mode = m.q7;
m.zoom = (1+(0.7*Math.sin((m.time*1.51))));
		m.rkeys = ['zoom'];
		return m;
	},
	'pixel_eqs' : function(m) {
m.rot = ifcond(above(m.q4, 0), ((m.rad*0.2)*m.q5), Math.sin((m.rad*(m.q4+m.q3))));
m.zoom = ifcond(above(m.q2, 0), (m.zoom+(((1-m.zoom)*m.rot)*Math.cos(((m.rad*3.14)*m.q2)))), ifcond((above(m.q3, 0)*above(m.x, (0.5+(0.5*m.q5)))), (m.zoom+((1-m.zoom)*Math.sin(((m.q1*m.rot)*3.14)))), (m.zoom+((1-m.zoom)*Math.cos(((m.rad*10)*m.q6))))));
m.dx = (above(m.q1, 0)*Math.sin(((m.rad*0.5)*m.q2)));
m.dy = (above(m.q3, 0)*Math.sin(((m.rad*0.5)*m.q3)));
		return m;
	},
	'warp' : (''),
	'comp' : (''),
	...
	'pixel_eqs_str' : ('rot=if(above(q4,0),rad*.2*q5,sin(rad*(q4+q3)));\n' + 'zoom=if(above(q2,0),zoom+(1-zoom)*rot*cos(rad*3.14*q2),if(above(q3,0)*above(x,.5+.5*q5),zoom+(1-zoom)*sin(q1*rot*3.14),zoom+(1-zoom)*cos(rad*10*q6)));\n' + 'dx=above(q1,0)*sin(rad*.5*q2);\n' + 'dy=above(q3,0)*sin(rad*.5*q3);'),
};
