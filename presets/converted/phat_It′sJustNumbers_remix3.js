...
m.decay = 0.9999;
m.zoom = -0.999;
m.rot = (Math.sin(div(m.time,10))*0.02);
m.ib_b = (Math.sin(div(m.time,20))+(0.5*0.5));
m.ib_g = ((Math.cos(div(m.time,15))*0.3)+(0.5*0.5));
m.ib_r = ((Math.sin(div(m.time,17))*0.3)+(0.5*0.5));
m.ob_b = (Math.sin((div(m.time,17)+0.9))+(0.5*0.5));
m.ob_g = Math.sin((div(m.time,20)+0.1));
m.ob_r = (Math.cos(div(m.time,15))+0.3);
		m.rkeys = [];
		return m;
	},
	'pixel_eqs' : function(m) {
m.xtc = (Math.sin(div(m.time,10))*10);
...
m.y = ((Math.sin(m.time)*0.3)+0.5);
	...
	'frame_eqs_str' : ('decay=0.9999;\n' + 'zoom=-.999;\n' + 'rot=sin(time/10)*0.02;\n' + 'ib_b=(sin(time/20))+0.5*0.5;\n' + 'ib_g=(cos(time/15)*0.3)+0.5*0.5;\n' + 'ib_r=(sin(time/17)*0.3)+0.5*0.5;\n' + 'ob_b=(sin((time/17)+0.9)+0.5*0.5);\n' + 'ob_g=sin((time/20)+0.1);\n' + 'ob_r=(cos(time/15))+0.3;'),
	'pixel_eqs_str' : ('xtc=sin(time/10)*10;\n' + 'speed=40-above(bass,0.9)*15;\n' + 'speed=speed+xtc;\n' + 'acid=x*sin(time/10);\n' + 'morphine=y*cos(time/10);\n' + 'dmt=above((treb*treb*treb/3),2.5)*.01;\n' + 'saliva=above(treb,0.9)*.01;\n' + 'cx=dmt*(rad)*0.1;\n' + 'cy=saliva*(rad)*0.1;\n' + 'dx=sin(y*speed)*0.01;\n' + 'dy=cos(x*speed)*0.01;\n' + 'zoom=cos(x*(sin(time/10)*0.1))+(y*(cos(time/10)*0.1));\n' + 'warp=-1*(((sin(time/2)*0.5+0.5)*3)*x)+(((cos(time/2)*0.5+0.5)*3)*y);'),
};
