...
m.echo_alpha = 0.5;
m.echo_zoom = ((Math.sin(m.q2)*0.2)+1);
		m.rkeys = [];
		return m;
	},
...
m.t2 = 0;
m.t3 = 0;
m.time1 = (1000*m.bass_att);
m.time2 = (1000*m.treb_att);
			m.rkeys = [];
			return m;
		},
		'frame_eqs' : function(m) {
m.time1 = ((m.time1+((((Math.sin(((m.q2*m.q4)*0.15))*0.4)+0.5)*0.001)*m.q4))+(m.q1*m.bass));
m.time2 = ((m.time2+((((Math.cos(((m.q2*m.q4)*0.1))*0.4)+0.5)*0.001)*m.q4))+(m.q1*m.treb));
m.t1 = m.time1;
m.t2 = m.time2;
m.hue = (0.1*m.q2);
...
m.t3 = (((m.sw1+(m.sw2*(2-m.h)))+(m.sw5*(m.h-4)))+m.sw6);
m.t4 = ((((m.sw1*m.h)+m.sw2)+m.sw3)+(m.sw4*(4-m.h)));
m.t5 = ((((m.sw3*(m.h-2))+m.sw4)+m.sw5)+(m.sw6*(6-m.h)));
		return m;
	},
		'point_eqs' : function(m) {
m.i = (m.sample*6.28);
m.x1 = (Math.sin((m.i+m.t2))*Math.sin((m.i*m.t1)));
m.y1 = (Math.cos((m.i+m.t1))*Math.sin((m.i*m.t2)));
m.x2 = ((m.x1*0.5)+0.5);
m.y2 = ((m.y1*0.5)+0.5);
m.x = m.x2;
m.y = m.y2;
...
m.b = (m.t5*(0.5-m.ii));
		...
		'init_eqs_str' : ('time1=1000*bass_att;\n' + 'time2=1000*treb_att;'),
		'frame_eqs_str' : ('time1=time1+(sin(q2*q4*.15)*.4+.5)*.001*q4+q1*bass;\n' + 'time2=time2+(cos(q2*q4*.1)*.4+.5)*.001*q4+q1*treb;\n' + 't1=time1;\n' + 't2=time2;\n' + 'hue=.1*q2;\n' + 'h=6*(hue-int(hue));\n' + 'sw1=below(h,1);\n' + ' sw2=(1-sw1)*below(h,2);\n' + ' sw3=(1-sw1)*(1-sw2)*below(h,3);\n' + ' sw4=(1-sw1)*(1-sw2)*(1-sw3)*below(h,4);\n' + 'sw6=above(h,5);\n' + ' sw5=(1-sw1)*(1-sw2)*(1-sw3)*(1-sw4)*(1-sw6);\n' + 't3=sw1+sw2*(2-h)+sw5*(h-4)+sw6;\n' + 't4=sw1*h+sw2+sw3+sw4*(4-h);\n' + 't5=sw3*(h-2)+sw4+sw5+sw6*(6-h);'),
		'point_eqs_str' : ('i=sample*6.28;\n' + 'x1=sin(i+t2)*sin(i*t1);\n' + 'y1=cos(i+t1)*sin(i*t2);\n' + 'x2=x1*.5+.5;\n' + 'y2=y1*.5+.5;\n' + 'x=x2;\n' + 'y=y2;\n' + 'ii=sample;\n' + 'r=t3*ii*2;\n' + 'g=t4*(1-ii);\n' + 'b=t5*(.5-ii);'),
