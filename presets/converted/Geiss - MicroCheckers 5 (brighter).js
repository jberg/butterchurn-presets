...
m.wave_r = (m.wave_r+(0.400*((0.60*Math.sin((1.517*m.time)))+(0.40*Math.sin((1.580*m.time))))));
m.wave_g = (m.wave_g+(0.400*((0.60*Math.sin((1.088*m.time)))+(0.40*Math.sin((1.076*m.time))))));
...
m.q2 = (m.time+(0.3*Math.sin((m.time*1.47))));
	...
	'warp' : ('shader_body {\n' + '   vec3 ret_1;\n' + '   vec2 tmpvar_2;\n' + '  tmpvar_2 = (uv_orig * texsize.xy);\n' + '   vec2 tmpvar_3;\n' + '  tmpvar_3 = mix (uv, ((\n' + '    (uv_orig - 0.5)\n' + '   * 0.97) + 0.5), vec2(clamp ((2.0 * \n' + '    fract((((tmpvar_2.x + tmpvar_2.y) * 0.5) + 0.25))\n' + '  ), 0.0, 1.0)));\n' + '   vec4 tmpvar_4;\n' + '  tmpvar_4 = texture2D (sampler_main, tmpvar_3);\n' + '  ret_1 = tmpvar_4.xyz;\n' + '  ret_1 = (ret_1 * (0.85 + (0.15 * \n' + '    clamp ((rad * 5.0), 0.0, 1.0)\n' + '  )));\n' + '   vec4 tmpvar_5;\n' + '  tmpvar_5.w = 1.0;\n' + '  tmpvar_5.xyz = ret_1;\n' + '  vec4 ret4 = tmpvar_5;\n' + '\n' + '  ret = ret4.xyz;\n}\n'),
	'comp' : ('shader_body {\n' + '   vec3 ret_1;\n' + '   vec4 tmpvar_2;\n' + '  tmpvar_2 = texture2D (sampler_main, uv);\n' + '  ret_1 = tmpvar_2.xyz;\n' + '  ret_1 = (mix (vec3(dot (ret_1, vec3(0.32, 0.49, 0.29))), ret_1, vec3(3.0, 3.0, 3.0)) * 4.0);\n' + '   vec4 tmpvar_3;\n' + '  tmpvar_3.w = 1.0;\n' + '  tmpvar_3.xyz = ret_1;\n' + '  vec4 ret4 = tmpvar_3;\n' + '\n' + '  ret = ret4.xyz;\n}\n'),
	'init_eqs_str' : (''),
	'frame_eqs_str' : ('wave_r = wave_r + 0.400*( 0.60*sin(1.517*time) + 0.40*sin(1.580*time) );\n' + 'wave_g = wave_g + 0.400*( 0.60*sin(1.088*time) + 0.40*sin(1.076*time) );\n' + 'wave_b = wave_b + 0.200*( 0.60*sin(1.037*time) + 0.40*sin(0.922*time) );\n' + 'rot = rot + 0.020*( 0.60*sin(0.381*time) + 0.40*sin(0.579*time) );\n' + 'cx = cx + 0.110*( 0.60*sin(0.374*time) + 0.40*sin(0.294*time) );\n' + 'cy = cy + 0.110*( 0.60*sin(0.393*time) + 0.40*sin(0.223*time) );\n' + 'q1=cos(1.41*time);\n' + 'q2=time + 0.3*sin(time*1.47);'),
	'pixel_eqs_str' : (''),
};
