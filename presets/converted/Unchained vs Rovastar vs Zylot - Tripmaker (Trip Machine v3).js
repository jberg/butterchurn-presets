...
m.monitor = m.q2;
m.mv_a = (m.q1*0.02);
		m.rkeys = ['rot','cy','cx','zoom'];
		return m;
	},
	'pixel_eqs' : function(m) {
m.zoom = (m.zoom+(0.1*Math.sin(((m.ang*(m.q2+2))-m.time))));
m.zoomexp = (100*(sqrt(2)-Math.sin(((m.rad*3.14)*m.q2))));
m.cx = (m.cx*m.zoom);
m.cy = (m.cy*m.zoom);
m.rot = bitand((m.rot+0),Math.sin(bitand(bitand(0,((m.ang*20)+0)),(m.time*10))));
m.zoom = (m.zoom+Math.abs((m.rot-m.rad)));
		return m;
	},
	'waves' : [
		{
...
m.g2 = (0.5+(0.49*Math.sin((m.time*0.7714))));
	...
	'pixel_eqs_str' : ('zoom = zoom + .1*sin((ang)*(q2+2)-time);\n' + 'zoomexp = 100*(sqrt(2)-sin(rad*3.14*q2));\n' + 'cx=cx*zoom;\n' + 'cy=cy*zoom;\n' + 'rot = rot + 0&sin(0&(ang*20)+0&(time*10));\n' + 'zoom = zoom + abs(rot-rad);'),
};
