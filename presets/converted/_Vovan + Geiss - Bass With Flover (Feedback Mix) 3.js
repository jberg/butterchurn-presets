...
m.zoom = (m.zoom+(0.07*Math.sin(((m.rad*25)+(m.time*2.5)))));
	...
	'shapes' : [
		...
		'frame_eqs' : function(m) {
m.x = (m.x+(0.03*Math.cos(((m.time*0.69)+2))));
m.y = (m.y+(0.02*Math.cos(((m.time*0.73)+1))));
		return m;
	},
		'init_eqs_str' : (''),
		'frame_eqs_str' : ('x = x + 0.03*cos(time*0.69 + 2);\n' + 'y = y + 0.02*cos(time*0.73 + 1);'),
