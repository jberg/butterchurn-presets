...
m.q1 = (0.5+((0.1*Math.cos((m.time*0.24)))*Math.sin((m.time*0.68))));
m.q2 = (0.5+((0.1*Math.cos((m.time*0.17)))*Math.sin((m.time*0.88))));
m.q4 = (0.5+((0.5*Math.sin((m.time*0.52)))*Math.cos((m.time*0.39))));
m.q5 = (0.5+((0.05*Math.sin((m.time*0.44)))*Math.cos((m.time*0.71))));
		m.rkeys = ['sx','sy','zoom'];
		return m;
	},
	'pixel_eqs' : function(m) {
m.zoom = ((m.zoom+(0.015*m.rad))+((0.025*m.treb_att)*m.rad));
m.sy = (m.sy+(0.1*Math.cos((Math.sin(((m.y*40)+m.time))-((Math.sin(((20*m.x)+m.time))*0.5)*m.treb_att)))));
m.sx = (m.sx+(0.025*Math.sin((Math.cos(((m.x*40)+m.time))+((Math.cos(((m.y*20)+m.time))*0.5)*m.treb_att)))));
		return m;
	},
	'waves' : [
		{
...
m.b = (m.treb*0.7);
	...
	'warp' : ('shader_body {\n' + '   vec3 ret_1;\n' + '   vec3 tmpvar_2;\n' + '  tmpvar_2 = texture2D (sampler_main, uv).xyz;\n' + '  ret_1 = tmpvar_2;\n' + '   vec3 tmpvar_3;\n' + '  tmpvar_3 = (0.15 * texture2D (sampler_noise_hq, uv)).xyz;\n' + '  ret_1 = (ret_1 * (vec3(0.9, 0.9, 9.0) + tmpvar_3));\n' + '  ret_1 = (ret_1 - 0.002);\n' + '   vec4 tmpvar_4;\n' + '  tmpvar_4.w = 1.0;\n' + '  tmpvar_4.xyz = ret_1;\n' + '  vec4 ret4 = tmpvar_4;\n' + '\n' + '  ret = ret4.xyz;\n}\n'),
	'comp' : ('shader_body {\n' + '   vec3 fret_1;\n' + '   vec3 tret_2;\n' + '   vec3 pret_3;\n' + '   vec2 uvpm_4;\n' + '   vec2 uvp_5;\n' + '   vec3 ret_6;\n' + '  uvp_5.x = uv.x;\n' + '  uvp_5.y = (uv.y * 0.38);\n' + '   vec2 tmpvar_7;\n' + '  tmpvar_7 = vec2(((uv.x - 0.5) + _qa.y));\n' + '   vec4 tmpvar_8;\n' + '  tmpvar_8 = texture2D (sampler_noise_hq, tmpvar_7);\n' + '  uvpm_4.x = (uv.x * tmpvar_8.x);\n' + '   vec2 tmpvar_9;\n' + '  tmpvar_9 = vec2(((uvp_5.y - 0.5) + _qa.x));\n' + '   vec4 tmpvar_10;\n' + '  tmpvar_10 = texture2D (sampler_noise_hq, tmpvar_9);\n' + '  uvpm_4.y = (uvp_5.y * tmpvar_10.x);\n' + '   vec4 tmpvar_11;\n' + '  tmpvar_11 = texture2D (sampler_blur2, uvp_5);\n' + '   vec4 tmpvar_12;\n' + '  tmpvar_12 = texture2D (sampler_blur3, uvp_5);\n' + '   vec3 tmpvar_13;\n' + '  tmpvar_13 = ((tmpvar_12.xyz * scale3) + bias3);\n' + '   vec4 tmpvar_14;\n' + '  tmpvar_14 = texture2D (sampler_blur2, uv);\n' + '   vec3 tmpvar_15;\n' + '  tmpvar_15 = texture2D (sampler_main, uvp_5).xyz;\n' + '  pret_3 = tmpvar_15;\n' + '   vec3 tmpvar_16;\n' + '  tmpvar_16 = texture2D (sampler_main, uvpm_4).xyz;\n' + '  tret_2 = tmpvar_16;\n' + '  fret_1 = (((0.17 * tmpvar_13) / (pret_3 * tmpvar_13)).xxx - ((\n' + '    (tmpvar_14.xyz * scale2)\n' + '   + bias2) * 1.35));\n' + '  ret_6 = ((fret_1 * tret_2) * (1.0 - (tret_2 * tret_2)));\n' + '  ret_6 = (ret_6 * (1.0 - (\n' + '    (0.5 * fret_1)\n' + '   * \n' + '    (1.0 - (4.0 * ret_6))\n' + '  )));\n' + '  ret_6 = (1.0 - ret_6);\n' + '  ret_6 = (ret_6 * (ret_6 * dot (\n' + '    (ret_6 - ((tmpvar_11.xyz * scale2) + bias2))\n' + '  , vec3(0.32, 0.49, 0.29))));\n' + '  ret_6 = (1.0 - ret_6);\n' + '   vec4 tmpvar_17;\n' + '  tmpvar_17.w = 1.0;\n' + '  tmpvar_17.xyz = ret_6;\n' + '  vec4 ret4 = tmpvar_17;\n' + '\n' + '  ret = ret4.xyz;\n}\n'),
	'init_eqs_str' : ('decay=0.1;'),
	'frame_eqs_str' : ('vol=(bass+mid+treb_att)*.3333;\n' + 'redsine=.5+.15*bass*sin(time*3);\n' + 'greensine=.5+.15*mid*sin(time*2);\n' + 'bluesine=.5+.15*treb*sin(time);\n' + 'redif=if(above(bass,1.2),redsine,if(above(redif,.95),0,redif*.85));\n' + 'greenif=if(above(mid,1.2),greensine,if(above(greenif,.95),0,greenif*.85));\n' + 'blueif=if(above(treb,1.2),bluesine,if(above(blueif,.95),0,blueif*.85));\n' + 'q1=.5+.1*cos(time*.24)*sin(time*.68);\n' + 'q2=.5+.1*cos(time*.17)*sin(time*.88);\n' + 'q4=.5+.5*sin(time*.52)*cos(time*.39);\n' + 'q5=.5+.05*sin(time*.44)*cos(time*.71);'),
	'pixel_eqs_str' : ('zoom=zoom+.015*rad+.025*treb_att*rad;\n' + 'sy=sy+.1*cos(sin(y*40+time)-sin(20*x+time)*.5*treb_att);\n' + 'sx=sx+.025*sin(cos(x*40+time)+cos(y*20+time)*.5*treb_att);'),
};
