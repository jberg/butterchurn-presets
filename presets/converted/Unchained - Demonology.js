...
m.grid = (pow((Math.sin((((m.rad*6)*Math.sin(m.q8))+(((m.x*m.y)*16)*m.q1)))*2), (1+mod(m.q8,3)))*(1+m.q7));
m.rot = ((m.snee*bnot((below(m.y, 0.5)*below(m.q7, 3))))*ifcond(bnot(mod(m.grid,m.q8)), 1, (0.1*Math.sin(((m.rad*3.14)*m.q3)))));
m.zoom = ((m.zoom-((0.031*m.snur)*Math.sin(((m.rad*m.q7)*m.q5))))+(((m.snee*bnot(m.snur))*Math.sin(((m.rad*6)*m.q5)))*0.1));
m.sx = (m.sx+((0.0361*bnot(m.snee))*Math.cos(((m.y*3.14)*m.q4))));
m.sy = (m.sy+((0.00361*bnot(m.snur))*Math.cos(((m.x*3.14)*m.q6))));
		return m;
	},
	'warp' : (''),
	'comp' : (''),
	...
	'pixel_eqs_str' : ('snee=bnot(above(x,.5)*above(q2,0)+above(y,.5)*above(q1,0));\n' + 'snur=bnot(below(x,.5)*above(q3,0)+below(y,.5)*below(q7,4));\n' + 'grid=pow(sin(rad*6*sin(q8)+x*y*16*q1)*2,1+q8%3)*(1+q7);\n' + 'rot=snee*bnot(below(y,.5)*below(q7,3))*if(bnot(grid%q8),1,.1*sin(rad*3.14*q3));\n' + 'zoom=zoom-.031*snur*sin(rad*q7*q5)+snee*bnot(snur)*sin(rad*6*q5)*.1;\n' + 'sx=sx+.0361*bnot(snee)*cos(y*3.14*q4);\n' + 'sy=sy+.00361*bnot(snur)*cos(x*3.14*q6);'),
};
