...
m.q3 = Math.min(m.q3, 1.0);
m.monitor = div(Math.floor((m.q3*4)),4);
m.q4 = m.mt;
m.q5 = m.bt;
m.cx = m.q1;
m.cy = m.q2;
...
m.my = (m.y-m.b);
m.zm = -0.45;
m.zm = ifcond(above(Math.sin((m.time*2.1)), 0.95), (0.45*Math.max((1-(m.q3*0.5)), 0)), m.zm);
...
m.dsign = sign(m.dx);
m.dx = (m.dx-(Math.max((Math.abs(m.dx)-m.limit), 0)*m.dsign));
m.dy = ((((m.zm*m.my)*m.dis)*Math.cos((m.mx*3.14)))*m.q3);
m.dsign = sign(m.dy);
m.dy = (m.dy-(Math.max((Math.abs(m.dy)-m.limit), 0)*m.dsign));
		return m;
	},
	'waves' : [
		{
