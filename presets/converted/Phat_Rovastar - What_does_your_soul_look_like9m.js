...
m.rot = div((Math.sin(div(m.time,(m.rad*0.5)))*m.x),div(div(div(Math.cos(div(m.time,m.rad)),div(m.ang,3)),m.x),(((Math.sin(div(m.time,6))*0.5)+0.5)*m.y)));
m.warp = (div((Math.cos(div(m.time,10))*m.y),((div(Math.sin(div(m.time,6)),(m.rad*((Math.sin(div(m.time,3))*0.5)+0.5)))*m.y)*m.x))*3);
		return m;
	},
	'waves' : [
		{
	...
	'shapes' : [
		...
		'frame_eqs' : function(m) {
m.rad = (0.200+(div(((m.bass_att+m.mid_att)+m.treb_att),3)*0.100));
		return m;
	},
		'init_eqs_str' : (''),
		'frame_eqs_str' : ('rad=.200+(((bass_att+mid_att+treb_att)/3)*.100);'),
