...
m.rot = (m.rot+(0.15*((0.60*Math.sin((0.381*m.time)))+(0.40*Math.sin((0.479*m.time))))));
m.mytime = 0.9;
m.xpos = div(0.5,m.vol);
m.ypos = div(0.5,m.vol);
m.q1 = Math.sin(((m.time*m.mytime)*4));
m.q2 = Math.cos(((m.time*m.mytime)*2));
m.q3 = (Math.abs((m.rad+0.5))+(m.q2*m.q5));
m.q4 = ((Math.abs((0.9*5))*(m.dx_r*m.dy_r))*(m.dx*m.dy));
m.q5 = (Math.cos((0.2*2))*(m.dx_r*m.dy_r));
m.q6 = ((0.1*Math.atan(Math.abs((-m.rad+0.5))))*m.q1);
m.q7 = (((((m.q1+m.q2)+m.q3)+m.q4)+m.q5)+m.q6);
...
m.zoom = ((ifcond(below(m.q1, m.q6), (0.5*m.x), ifcond(equal(m.q2, m.q5), (0.9*(1-m.x)), ifcond(above(m.q3, m.q4), (0.5*m.y), ((0.9*(1-m.y))+ifcond(below(m.q4, m.q3), (0.5*m.xpos), ifcond(equal(m.q5, m.q2), (0.9*(1-m.xpos)), ifcond(above(m.q6, m.q1), (0.9*m.ypos), (0.3*(1-m.ypos)))))))))+0.8)-(0.03*Math.min((m.q7+m.q1), 1)));
		return m;
	},
	'warp' : (''),
	'comp' : (''),
	...
	'pixel_eqs_str' : ('zoom= if(below(q1,q6),0.5*x,if(equal(q2,q5),0.9*(1-x),if(above(q3,q4),0.5*y,0.9*(1-y)+if(below(q4,q3),0.5* xpos, if(equal(q5,q2),0.9*(1-xpos),if(above(q6,q1),0.9*ypos,0.3*(1-ypos))))))) + .8 - 0.03*(min(q7+q1,1));'),
};
