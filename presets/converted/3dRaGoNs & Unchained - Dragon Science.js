...
m.rot = (((bnot((above(m.x, 0.5)+mod((m.y*10),5)))*Math.cos((m.rad+((3.14*ifcond(above(m.grid, 0), m.snur, bnot(m.snur)))*(0.5+(0.5*Math.sin(((m.rad*3.14)*m.q1))))))))*m.q6)*0.3);
m.sx = (m.sx-((bor(m.snee, m.snur)*Math.cos((m.y*m.q2)))*0.05));
m.sy = (m.sy-((bor(bnot(m.snee), m.snur)*Math.cos((m.x*m.q1)))*0.03));
m.cx = (m.cx+((Math.sin(((m.x*3.14)*m.q6))*m.snur)*0.45));
m.cy = (m.cx+((Math.sin(((m.y*3.14)*m.q5))*m.snee)*0.5));
m.zoom = ((m.zoom+((0.1*m.snee)*bnot(m.snur)))-((0.1*m.snur)*bnot(m.snee)));
		return m;
	},
	'warp' : (''),
	'comp' : (''),
	...
	'pixel_eqs_str' : ('snee=bnot(above(sin(ang)-x,.5)*above(q2,0)+above(y-cos(ang),.5)*above(q1,0));\n' + 'snur=bnot(below(x,.5)*above(q3,0)+below(y,.5)*below(q7,4));\n' + 'grid=sin(sigmoid(sin(y*6.28*q2),sin(x*6.28*q6))*(10+q7));\n' + 'rot=bnot(above(x,.5)+y*10%5)*cos(rad+3.14*if(above(grid,0),snur,bnot(snur))*(.5+.5*sin(rad*3.14*q1)))*q6*.3;\n' + 'sx=sx-bor(snee,snur)*cos(y*q2)*.05;\n' + 'sy=sy-bor(bnot(snee),snur)*cos(x*q1)*.03;\n' + 'cx=cx+sin(x*3.14*q6)*snur*.45;\n' + 'cy=cx+sin(y*3.14*q5)*snee*.5;\n' + 'zoom=zoom+.1*snee*bnot(snur)-.1*snur*bnot(snee);'),
};
