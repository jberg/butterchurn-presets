...
m.warp = 0;
		m.rkeys = ['rot','sy','sx','dx','dy','zoom','dy_r','dx_r','thresh'];
		return m;
	},
...
m.zoom = (m.zoom+0.08);
m.zoom = (m.zoom-((0.5-(((m.y*m.rad)*0.2)*Math.abs(Math.cos(((m.rad*m.bass)-(0.7*Math.sin((m.ang*12))))))))*0.1));
m.dy = (m.dy+m.dy_r);
m.dx = (m.dx+m.dx_r);
m.sx = ((m.sx+(Math.sin(m.time)*(0.4*m.rad)))-(Math.sin(((((3*m.bass)*Math.cos((m.ang*16)))*0.3)-m.rad))*0.05));
m.sy = ((m.sy+(Math.cos((1.25*m.time))*(0.4*m.rad)))-(Math.cos(((((3*m.treb)*Math.sin((m.ang*16)))*0.3)-m.rad))*0.05));
m.rot = ((m.rot-(Math.tan(m.time)*(0.3*m.rad)))-(Math.sin((((m.mid*Math.sin((m.ang*6)))*0.3)-m.rad))*0.1));
		return m;
	},
	'warp' : (''),
	'comp' : (''),
	...
	'pixel_eqs_str' : ('thresh = above(bass_att,thresh)*2+(1-above(bass_att,thresh))*((thresh-1.3)*0.96+1.3);\n' + 'dx_r = equal(thresh,2)*0.015*sin(5*time)+(1-equal(thresh,2))*dx_r;\n' + 'dy_r = equal(thresh,2)*0.015*sin(6*time)+(1-equal(thresh,2))*dy_r;\n' + 'zoom = zoom + 0.08;\n' + 'zoom = zoom - (0.5-y*rad*0.2*abs(cos(rad*bass-0.7*sin(ang*12))))*0.1;\n' + 'dy = dy + dy_r;\n' + 'dx = dx + dx_r;\n' + 'sx = sx + sin(time)*(0.4*rad)-sin(3*bass*cos(ang*16)*0.3-rad)*0.05;\n' + 'sy = sy + cos(1.25*time)*(0.4*rad)-cos(3*treb*sin(ang*16)*0.3-rad)*0.05;\n' + 'rot = rot - tan(time)*(0.3*rad)-sin(mid*sin(ang*6)*0.3-rad)*0.1;'),
};
